...
<thead>
    ...
    <th class='hidden-xs hidden-sm'>READONLY</th>
    <th class='hidden-xs hidden-sm'>DBA</th>
    ...
    if (api_success(data)){
        if (data.data){
            $.each(data.data, function(rowid,row){
				...
				id = "NODES-" + row["NODES_ID"] + "-NODES_ASSETS_ID";
                td = $('<td>').addClass("TD-EDIT").addClass("hidden-xs hidden-sm");
                select = $('<select/>').attr("id",id).addClass("TABLE-EDIT");
                $.each(ASSETS, function(key, row){
					$("<option />", {value: row["ASSETS_ID"], text: row["ASSETS_NAME"]}).appendTo(select);
				});
                $(select).val(row["NODES_ASSETS_ID"]);
                $(td).append(select);
                $(tr).append(td);
...
load_assets_callback = function(data){
    if (api_success(data)){
        if (data.data){ ASSETS = data.data; }
        load_table();
    }
}

load_assets = function(){
    api("ASSETS/list", { }, load_assets_callback);
}
