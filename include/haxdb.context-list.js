...
save_query = function(){
	var data = {
		"QUERY_QUERY": $("#CONTEXT-SEARCH").val(),
		"context": context,
		"context_id": context_id,
		"rowid": CURRENT_QUERY
	}
...
draw_table = function() {
    ...
    if (CONTEXT_DATA) {
        $.each(CONTEXT_DATA, function(rowid, row) {
            ...
            if (row[context + "_INTERNAL"] && row[context + "_INTERNAL"] == 1) {
                $(td).addClass("disabled");
            }
            delete_name = "";
...
$(document).on("click",".CONTEXT-ROW-DELETE:not(.disabled)", function(){
  ...
  $.each(deletenames, function(key, col){
    ...
    rowname += CONTEXT_DATA[rowid][col];
  });
